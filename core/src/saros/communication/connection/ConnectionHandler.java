...
import saros.net.XMPPConnectionState;
...
public class XMPPConnectionHandler {
    ...
    private static final Logger LOG = Logger
        .getLogger(XMPPConnectionHandler.class);
    ...
    private final IConnectionListener xmppConnectionListener = new IConnectionListener() {
        ...
        public void connectionStateChanged(XMPPConnection connection,
            ...
            final Exception error = state == ConnectionState.ERROR
    ...
    public XMPPConnectionHandler(final XMPPConnectionService connectionService,
    ...
    private void connectXMPPInternal(final XMPPAccount account,
        ...
        try {
            ...
            connectionService.connect(createConnectionConfiguration(domain,
                server, port, useTLS, useSASL), username, password);
        } catch (Exception e) {
    ...
    private XMPPConnectionConfiguration createConnectionConfiguration(
        ...
        XMPPConnectionConfiguration connectionConfiguration = null;

        if (server.length() == 0 && proxyInfo == null)
            connectionConfiguration = new XMPPConnectionConfiguration(domain);
        else if (server.length() == 0 && proxyInfo != null)
            connectionConfiguration = new XMPPConnectionConfiguration(domain,
                proxyInfo);
        else if (server.length() != 0 && proxyInfo == null)
            connectionConfiguration = new XMPPConnectionConfiguration(server,
                port, domain);
        else
            connectionConfiguration = new XMPPConnectionConfiguration(server,
                port, domain, proxyInfo);

        connectionConfiguration.setSASLAuthenticationEnabled(useSASL);

        if (!useTLS)
            connectionConfiguration.setSecurityMode(
                XMPPConnectionConfiguration.SecurityMode.disabled);
