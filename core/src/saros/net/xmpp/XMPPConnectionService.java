...
import org.jivesoftware.smack.XMPPConnectionConfiguration;
...
public class XMPPConnectionService {
    ...
    private final XMPPConnectionListener smackConnectionListener = new XMPPConnectionListener() {
        ...
        public void connectionClosedOnError(Exception e) {
            synchronized (XMPPConnectionService.this) {
                ...
                setConnectionState(XMPPConnectionState.ERROR, e);
                disconnectInternal();
                setConnectionState(XMPPConnectionState.NOT_CONNECTED, null);
                localJID = null;
            }
        }
    ...
    public XMPPConnectionService(@Nullable IUPnPService upnpService,
        ...
        packetReplyTimeout = Integer
            .getInteger("saros.net.smack.PACKET_REPLAY_TIMEOUT", 30000);
    }
    ...
    public synchronized void configure(final String resource,
        ...
        XMPPConnection.DEBUG_ENABLED = enableDebug;
        ...
        ThreadUtils.runSafeAsync("upnp-resolver", LOG, new Runnable() {
            @Override
            public void run() {
                if (upnpService != null)
     ...
     * server.</br>
     * The JID is also available in the states {@linkplain ConnectionState#ERROR
     * error}, {@linkplain ConnectionState#DISCONNECTING disconnecting} and
    ...
    public Roster getRoster() {
        return isConnected() ? Roster.getInstanceFor(connection) : null;
    }
     ...
     *            {@link XMPPConnectionConfiguration} Configuration for
    ...
    public synchronized void connect(
        XMPPConnectionConfiguration connectionConfiguration, String username,
        ...
        try {
            setConnectionState(XMPPConnectionState.CONNECTING, null);
            ...
            setConnectionState(XMPPConnectionState.CONNECTED, null);
    ...
    public synchronized void disconnect() {
        if (isConnected()) {
            setConnectionState(XMPPConnectionState.DISCONNECTING, null);
     ...
     * otherwise.
     *
     * @see IConnectionListener#connectionStateChanged(XMPPConnection,
     *      ConnectionState)
     ...
     *         none @Note it is strictly forbidden to call
     *         {@linkplain XMPPConnection#disconnect()} on the returned instance
     */
    ...
    /**
     * Configures Bytestream related settings, like ReplyTimeout, Socks5Proxy
     * configuration, look up streamhost address candidates for Socks5Proxy,
     * update UPnP settings on proxy setting change.
     */
    private void initialzeNetworkComponents() {
         ...
         * activities)
         */
        SmackConfiguration.setReplyTimeout(packetReplyTimeout);
        ...
        if (proxyAddresses != null) {
            ...
            if (interfaceAddresses.isEmpty())
                LOG.warn(
                    "Socks5 preconfigured addresses list is empty, using autodetect mode");
            else
                LOG.debug(
                    "using preconfigured addresses: " + interfaceAddresses);
        }
        ...
        if (stunService != null && stunServer != null) {
            ThreadUtils.runSafeAsync("stun-discovery", LOG, new Runnable() {
                @Override
                public void run() {
                    ...
                    for (InetSocketAddress address : addresses)
                        NetworkingUtils.addProxyAddress(
                            address.getAddress().getHostAddress(), true);
                }
            });
        }
        ...
        if (gatewayDeviceID != null && upnpService != null) {
            ...
            ThreadUtils.runSafeAsync("upnp-portmapping", LOG, new Runnable() {
                @Override
                public void run() {
                    synchronized (portMappingLock) {
                        ...
                        if (devices == null) {
                            LOG.warn(
                                "aborting UPNP port mapping due to network failure");
                            return;
                        }

                        for (GatewayDevice currentDevice : devices) {
                            if (gatewayDeviceIDToFind
                                .equals(currentDevice.getUSN())) {
    ...
    private void uninitialzeNetworkComponents() {
        ...
        deleteMapping: synchronized (portMappingLock) {
            ...
            if (!upnpService.isMapped(device, socks5ProxyPort,
                IUPnPService.TCP))
                break deleteMapping;

            LOG.debug(
                "deleting port mapping on device: " + device.getFriendlyName()
                    + " [" + socks5ProxyPort + "|" + IUPnPService.TCP + "]");
