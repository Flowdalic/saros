...
public class DiscoveryManager implements Disposable {
    ...
    private final ExecutorService threadPoolExecutor = new ThreadPoolExecutor(0,
        2, 30, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(),
    ...
    private final IRosterListener rosterListener = new IRosterListener() {
        ...
        private void clearCache(Collection<String> addresses) {
            for (String pjid : addresses) {
                 ...
                 * addresses
                 */
                for (Presence presence : rosterTracker
                    .getPresences(new JID(pjid))) {
                    clearCache(presence);
                }
            }
        }
        ...
        for (JID rqJID : jidsToQuery) {
            ...
            synchronized (wrapper) {
                if (wrapper.isAvailable())
                    ...
                    if (disco != null)
                        LOG.debug(
                            "Inserted DiscoveryInfo into Cache for: " + rqJID);
                }
            }
    ...
    private DiscoverInfo performServiceDiscovery(final JID jid) {
        ...
        if (connection == null) {
            LOG.warn(
                "cannot not perform a service discovery because not connected to a XMPP server");
            return null;
        }
