...
public class XMPPUtils {
    ...
    public static Registration createAccount(String server, String username,
        ...
        XMPPTCPConnection connection = new XMPPTCPConnection(server);
     ...
     */
    public static void removeFromRoster(XMPPConnection connection,
     ...
     */
    public static boolean isJIDonServer(XMPPConnection connection, JID jid,
    ...
    public static synchronized Registration getRegistrationInfo(
        XMPPConnection connection, String toRegister) throws XMPPException {
        Registration reg = new Registration();
        reg.setTo(connection.getXMPPServiceDomain());
        reg.setFrom(toRegister);
        StanzaFilter filter = new AndFilter(
            new StanzaIdFilter(reg.getStanzaId()),
            new StanzaTypeFilter(IQ.class));
        StanzaCollector collector = connection.createStanzaCollector(filter);

        final IQ result;

        try {
            connection.sendStanza(reg);
            result = (IQ) collector.nextResult();
     ...
     *         {@link UserSearch#getSearchForm(XMPPConnection, String)}
     */
    public static String getUserDirectoryService(XMPPConnection connection,
        String service) {
     ...
     */
    public static String getMultiUserChatService(XMPPConnection connection,
        String service) {
        ...
        while (iter.hasNext()) {
            DiscoverItems.Item item = iter.next();
            try {
                Iterator<Identity> identities = manager
    ...
    private static boolean isListedInUserDirectory(XMPPConnection connection,
        JID jid) {
